...
const STATUS_CONFIG = {
  ...
  ardha_sati: { color: '#F2CB84', bg: 'rgba(242,203,132,0.1)', label: 'Ardha Sati',   icon: '◐' },
...
function PhaseTag({ phase }) {
  if (phase?.type === 'sade_sati') {
    ...
    return <span style={{ fontSize: '0.72rem', background: 'rgba(224,123,57,0.2)', color: '#E07B39', border: '1px solid rgba(224,123,57,0.4)', borderRadius: 999, padding: '0.15rem 0.55rem', marginLeft: '0.5rem' }}>{label}</span>
  }
  if (phase?.type === 'ardha_sati') {
    return <span style={{ fontSize: '0.72rem', background: 'rgba(242,203,132,0.2)', color: GOLD_L, border: '1px solid rgba(242,203,132,0.4)', borderRadius: 999, padding: '0.15rem 0.55rem', marginLeft: '0.5rem' }}>Ardha Sati</span>
  }
  return null
}
...
function CycleCard({ cycle, isCurrent }) {
  ...
  return (
    <div style={{
      background: isCurrent ? 'rgba(223,168,79,0.07)' : 'var(--bg-elevated)',
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
        <span style={{ fontFamily: "'Fraunces', serif", fontSize: '0.92rem', letterSpacing: '-0.02em', color: isCurrent ? GOLD : 'var(--text-primary)', fontWeight: 600 }}>
          {start} – {end}
        </span>
        {isCurrent && <span style={{ fontSize: '0.62rem', color: GOLD, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '2px', textTransform: 'uppercase', border: `1px solid ${GOLD}`, borderRadius: 999, padding: '0.15rem 0.55rem' }}>Current</span>}
      </div>
      {cycle.phases?.map((p, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem', fontSize: '0.8rem', color: 'var(--text-dim)' }}>
          <span style={{ color: 'var(--text-dim)', width: '7ch', flexShrink: 0, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.74rem' }}>
...
function RemedyList({ remedies }) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 18, padding: '1.1rem 1.25rem', marginTop: '1.5rem', backdropFilter: 'blur(18px) saturate(1.4)', WebkitBackdropFilter: 'blur(18px) saturate(1.4)' }}>
      <button
        onClick={() => setOpen(o => !o)}
        style={{ background: 'none', border: 'none', color: GOLD, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.74rem', letterSpacing: '2.5px', textTransform: 'uppercase', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: 0, width: '100%', justifyContent: 'space-between' }}
      >
...
export default function SadeSati() {
  ...
  return (
    <div style={{ maxWidth: '820px', margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
      ...
      {/* Step 1: Sign picker */}
      {step === 'pick' && (
        <div className="bh-fade-up-1">
          <div style={{ fontSize: '0.66rem', color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '2.5px', textTransform: 'uppercase', marginBottom: '1rem', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '0.65rem' }}>
            {RASHIS.map((r, i) => (
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--border-hover)'; e.currentTarget.style.background = `${GOLD}12`; e.currentTarget.style.transform = 'translateY(-3px)' }}
              ...
              >
                ...
                <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.15rem' }}>{r.en}</div>
          ...
          {loading && (
            <div style={{ textAlign: 'center', marginTop: '2rem', color: 'var(--text-dim)' }}>
              <div style={{ fontSize: '1.5rem', animation: 'spin 2s linear infinite', display: 'inline-block', color: 'var(--gold)' }}>♄</div>
              <div style={{ marginTop: '0.5rem', fontSize: '0.88rem' }}>Computing Saturn transits…</div>
              <style>{`@keyframes spin { to { transform: rotate(360deg) } }`}</style>
            </div>
          )}
          {error && (
            <div style={{ marginTop: '1rem', background: 'rgba(229,57,53,0.1)', border: '1px solid rgba(229,57,53,0.3)', borderRadius: 10, padding: '0.85rem 1rem', color: '#ef9a9a', fontSize: '0.88rem' }}>{error}</div>
          )}

          {/* Info box */}
          <div className="bh-fade-up-2" style={{ marginTop: '2rem', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 18, padding: '1.25rem 1.4rem', backdropFilter: 'blur(18px) saturate(1.4)', WebkitBackdropFilter: 'blur(18px) saturate(1.4)' }}>
            <div style={{ fontSize: '0.66rem', color: GOLD, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '2.5px', textTransform: 'uppercase', marginBottom: '0.6rem' }}>Don't know your Moon sign?</div>
      ...
      {step === 'result' && result && (
        <div className="bh-fade-up-1">
          {/* Back */}
          <button onClick={reset} style={{ background: 'transparent', border: '1px solid var(--border-hover)', color: 'var(--text-muted)', fontSize: '0.8rem', cursor: 'pointer', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.35rem 0.9rem', borderRadius: 999 }}>
            ← Check another Rashi
          </button>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.2rem', marginBottom: '1.5rem' }}>
            ...
            <div>
              <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: '1.3rem', fontWeight: 600, letterSpacing: '-0.02em', color: 'var(--text-primary)', margin: '0 0 0.15rem' }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontFamily: "'Fraunces', serif", fontSize: '1.05rem', letterSpacing: '-0.02em', color: status.color, fontWeight: 600, marginBottom: '0.35rem' }}>{status.label}</div>
              ...
              {result.current_period && (
                <div style={{ marginTop: '0.75rem', fontSize: '0.78rem', color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace" }}>
          ...
          {result.next_ss && result.current_status !== 'sade_sati' && (
            <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 18, padding: '1rem 1.2rem', marginBottom: '1.75rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem', backdropFilter: 'blur(18px) saturate(1.4)', WebkitBackdropFilter: 'blur(18px) saturate(1.4)' }}>
              <div>
                <div style={{ fontSize: '0.64rem', color: GOLD, letterSpacing: '2.5px', textTransform: 'uppercase', fontFamily: "'JetBrains Mono', monospace", marginBottom: '0.2rem' }}>Next Sade Sati</div>
          ...
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.7rem', color: GOLD, letterSpacing: '3px', textTransform: 'uppercase', marginBottom: '1rem', paddingBottom: '0.5rem', borderBottom: '1px solid var(--border)' }}>
              Saturn Transit Timeline
            </div>

            {result.past_cycles?.length > 0 && (
              <div style={{ marginBottom: '1rem' }}>
                <div style={{ fontSize: '0.64rem', color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Past Cycles</div>
            ...
            {result.current_status === 'sade_sati' && result.current_period && (
              <div style={{ marginBottom: '1rem' }}>
                <div style={{ fontSize: '0.64rem', color: '#E07B39', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Current Cycle</div>
            ...
            {result.upcoming_cycles?.length > 0 && (
              <div>
                <div style={{ fontSize: '0.64rem', color: 'var(--text-dim)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '2px', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Upcoming Cycles</div>
          ...
          <div style={{ marginTop: '1.75rem', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 18, padding: '1.2rem 1.4rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem', backdropFilter: 'blur(18px) saturate(1.4)', WebkitBackdropFilter: 'blur(18px) saturate(1.4)' }}>
            <div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '0.66rem', color: GOLD, letterSpacing: '2.5px', textTransform: 'uppercase', marginBottom: '0.25rem' }}>Go deeper</div>
            ...
            <Link
              to="/chart/new"
              style={{ background: 'linear-gradient(135deg,#F2CB84 0%,#DFA84F 42%,#A8752B 100%)', color: '#1C1205', fontSize: '0.82rem', fontWeight: 600, letterSpacing: '0.02em', padding: '0.65rem 1.5rem', borderRadius: 999, boxShadow: '0 8px 28px rgba(223,168,79,0.28)', textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}
            >
              Free Chart →
            </Link>
          </div>
